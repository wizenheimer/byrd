...
type WorkspaceService interface {
	...
	CreateWorkspace(ctx context.Context, workspaceOwner *clerk.User, workspaceReq api.WorkspaceCreationRequest) (*models.Workspace, err.Error)
	...
	ListUserWorkspaces(ctx context.Context, workspaceMember *clerk.User) ([]models.Workspace, err.Error)

	// GetWorkspace gets a workspace by ID
	// It returns the workspace if it exists, otherwise it returns an error
	GetWorkspace(ctx context.Context, workspaceID uuid.UUID) (*models.Workspace, err.Error)

	// UpdateWorkspace updates a workspace
	// It returns an error if the workspace does not exist
	// It returns an error if the workspace is inactive
	UpdateWorkspace(ctx context.Context, workspaceID uuid.UUID, req api.WorkspaceUpdateRequest) err.Error

	// DeleteWorkspace deletes a workspace
	// It returns an error if the workspace does not exist
	// It returns an error if the workspace is inactive
	DeleteWorkspace(ctx context.Context, workspaceID uuid.UUID) (models.WorkspaceStatus, err.Error)
	...
	ListWorkspaceMembers(ctx context.Context, workspaceID uuid.UUID, params api.WorkspaceMembersListingParams) ([]models.WorkspaceUser, err.Error)
	...
	InviteUsersToWorkspace(ctx context.Context, workspaceMember *clerk.User, workspaceID uuid.UUID, invitedUsers []api.InviteUserToWorkspaceRequest) ([]api.CreateWorkspaceUserResponse, err.Error)
	...
	LeaveWorkspace(ctx context.Context, workspaceMember *clerk.User, workspaceID uuid.UUID) err.Error

	// UpdateWorkspaceMemberRole updates member role in a workspace
	// It returns an error if the user does not exist in the workspace
	UpdateWorkspaceMemberRole(ctx context.Context, workspaceID uuid.UUID, workspaceMemberID uuid.UUID, role models.UserWorkspaceRole) err.Error

	// RemoveUserFromWorkspace removes a user from a workspace
	// It returns an error if the user does not exist in the workspace
	RemoveUserFromWorkspace(ctx context.Context, workspaceID uuid.UUID, workspaceMemberID uuid.UUID) err.Error
	...
	JoinWorkspace(ctx context.Context, invitedMember *clerk.User, workspaceID uuid.UUID) err.Error
	...
	WorkspaceExists(ctx context.Context, workspaceID uuid.UUID) (bool, err.Error)
	...
	ClerkUserIsWorkspaceAdmin(ctx context.Context, workspaceID uuid.UUID, clerkUser *clerk.User) (bool, err.Error)
	...
	ClerkUserIsWorkspaceMember(ctx context.Context, workspaceID uuid.UUID, clerkUser *clerk.User) (bool, err.Error)
	...
	WorkspaceCompetitorPageExists(ctx context.Context, workspaceID, competitorID, pageID uuid.UUID) (bool, err.Error)
	...
	CreateWorkspaceCompetitor(ctx context.Context, clerkUser *clerk.User, workspaceID uuid.UUID, page api.CreatePageRequest) err.Error

	// AddPageToCompetitor adds a new page to an existing competitor
	// It returns an error if the workspace does not exist
	// It returns an error if the user is not an member of the workspace
	AddPageToCompetitor(ctx context.Context, clerkUser *clerk.User, competitorID string, pages []api.CreatePageRequest) ([]models.Page, err.Error)
	...
	ListWorkspacePageHistory(ctx context.Context, clerkUser *clerk.User, workspaceID, competitorID, pageID uuid.UUID, param api.PaginationParams) ([]models.PageHistory, err.Error)

	// RemovePage removes a page from a competitor
	// It returns an error if the workspace does not exist
	// It returns an error if the user is not an member of the workspace
	RemovePageFromWorkspace(ctx context.Context, clerkUser *clerk.User, competitorID, pageID uuid.UUID) err.Error

	// RemoveCompetitor removes a competitor from a workspace, including all its pages
	// It returns an error if the workspace does not exist
	// It returns an error if the user is not an member of the workspace
	RemoveCompetitorFromWorkspace(ctx context.Context, clerkUser *clerk.User, workspaceID, competitorID uuid.UUID) err.Error

	// UpdatePage updates a page
	// It returns an error if the workspace does not exist or page is not active
	// It returns an error if the user is not an member of the workspace
	// It returns an error if the page does not exist in the competitor of the workspace
	UpdateCompetitorPage(ctx context.Context, competitorID, pageID uuid.UUID, req api.UpdatePageRequest) err.Error
}
