...
type PageService interface {
	...
	CreatePage(ctx context.Context, competitorID uuid.UUID, pageReq []api.CreatePageRequest) ([]models.Page, err.Error)

	// GetPage gets a page by ID.
	// It returns the page if it exists, otherwise it returns an error.
	GetPage(ctx context.Context, competitorID uuid.UUID, pageID uuid.UUID) (models.Page, err.Error)

	// GetPageWithHistory gets a page along with its history.
	// It returns the page if it exists, otherwise it returns an error.
	GetPageWithHistory(ctx context.Context, competitorID uuid.UUID, pageID uuid.UUID, historyPaginationParams api.PaginationParams) (api.GetPageResponse, err.Error)

	// UpdatePage updates a page.
	// It returns the updated page and any errors that occurred.
	// This is triggered when a user updates a page.
	UpdatePage(ctx context.Context, competitorID uuid.UUID, pageID uuid.UUID, pageReq api.UpdatePageRequest) (models.Page, err.Error)
	...
	ListCompetitorPages(ctx context.Context, competitorID uuid.UUID, param *api.PaginationParams) ([]models.Page, err.Error)
	...
	ListPageHistory(ctx context.Context, competitorID uuid.UUID, pageID uuid.UUID, param api.PaginationParams) ([]models.PageHistory, err.Error)
	...
	RemovePage(ctx context.Context, competitorID uuid.UUID, pageIDs []uuid.UUID) err.Error
}
