...
type CompetitorService interface {
	...
	// or otherwise.
	CreateCompetitor(ctx context.Context, workspaceID uuid.UUID, competitorReq api.CreateCompetitorRequest) (api.CreateWorkspaceCompetitorResponse, errs.Error)
	...
	GetCompetitor(ctx context.Context, workspaceID, competitorID uuid.UUID, pagePaginationParam api.PaginationParams) (api.GetWorkspaceCompetitorResponse, errs.Error)

	// CompetitorExists checks if a competitor exists in a workspace.
	// Its optimized for quick lookups over the competitor table.
	CompetitorExists(ctx context.Context, workspaceID, competitorID uuid.UUID) (bool, errs.Error)

	// PageExists checks if a page exists in a competitor.
	// Its optimized for quick lookups over the page table.
	PageExists(ctx context.Context, competitorID, pageID uuid.UUID) (bool, errs.Error)
	...
	ListWorkspaceCompetitors(ctx context.Context, workspaceID uuid.UUID, competitorListingParam api.PaginationParams) ([]api.GetWorkspaceCompetitorResponse, errs.Error)
	...
	RemoveCompetitors(ctx context.Context, workspaceID uuid.UUID, competitorIDs []uuid.UUID) errs.Error
	...
	AddPagesToCompetitor(ctx context.Context, competitorID uuid.UUID, pageReq []api.CreatePageRequest) ([]models.Page, errs.Error)
	...
	GetCompetitorPage(ctx context.Context, competitorID, pageID uuid.UUID, pageHistoryPaginationParam api.PaginationParams) (api.GetPageResponse, errs.Error)
	...
	UpdatePage(ctx context.Context, competitorID, pageID uuid.UUID, pageReq api.UpdatePageRequest) (models.Page, errs.Error)
	...
	RemovePagesFromCompetitor(ctx context.Context, competitorID uuid.UUID, pageID []uuid.UUID) errs.Error
	...
	ListCompetitorPages(ctx context.Context, competitorID uuid.UUID) ([]models.Page, errs.Error)
}
